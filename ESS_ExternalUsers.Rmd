...
DT, ggplot2, knitr, leaflet, plotly, rworldmap, rworldxtra, sf, and tidyverse. 

*Note: you will need to download an older version of ggplot2 for this report to work. We are currently working on making this report compatible the the most recent ggplot2. In the meantime, download ggplot2 3.2.1 [here](https://cran.r-project.org/src/contrib/Archive/ggplot2/). Then, in your RStudio session, go to Tools -> Install Packages -> Install From: Package Archive File, and navigate to the zipped file you just downloaded. The file should be called ggplot2_3.2.1.tar.gz.*
...
Give the object "EcoSite" as Ecological Site Id in the section below:

```{r set ecosite, echo = TRUE, include = TRUE}
# This example is with 2 ecological sites. You can list as many as you want, or just 1. 
EcoSite <- c("R042XB012NM", "R042XB015NM")

```

```{r set terradat path , echo = FALSE , include = FALSE}

#Connect to TerrADat

#Set file path to TerrADat
#Since this is in the same folder as the R project/repository, just make sure the name is correct
#The path should automatically be set to your project

TerrADatPath <- "TerrADat.gdb"
...
TerrADat <- sf::st_read(dsn = TerrADatPath , layer = "TerrADat")

LMF <- sf::st_read(dsn = TerrADatPath , layer = "LMF")

TerrADat_Species <- sf::st_read(dsn = TerrADatPath , layer = "TerrADatSpeciesIndicators")

LMF_Species <- sf::st_read(dsn = TerrADatPath , layer = "LMFSpeciesIndicators")
...
TerrADat <- TerrADat %>% select(-Shape)
LMF <- as.data.frame(LMF)
LMF <- LMF %>% select(-Shape)
TerrADat_Species <- as.data.frame(TerrADat_Species)
TerrADat_Species <- TerrADat_Species %>% select(-Shape)
LMF_Species <- as.data.frame(LMF_Species)
LMF_Species <- LMF_Species %>% select(-Shape)
...
TDat_LMF_plots_Year <- TDat_LMF_plots_Year %>% rename(Year = DateVisited)


#Subset based on EcologicalSiteId
#EcoSitePlots <- TDat_LMF[TDat_LMF[["EcologicalSiteId"]] %in% EcoSite, ]
EcoSitePlots <- TDat_LMF %>% filter(EcologicalSiteId %in% EcoSite)
...
TDat_LMF_plots_Year_EDIT <- merge(TDat_LMF_plots_Year , EDIT_rename, by = "EcologicalSiteId", all = TRUE) #All =TRUE will keep AK plots with weird ESD 

#Filter out 0,0 lat lon

TDat_LMF_plots_Year_EDIT <- TDat_LMF_plots_Year_EDIT %>% filter(Latitude_NAD83 > 0)

EcoSiteList <- unique(TDat_LMF_plots_Year_EDIT$EcologicalSiteId)
...
EcoSiteCaption <-toString(EcoSite)
...
Year_Palette <- scales::seq_gradient_pal("grey85", "grey18", "Lab")(seq(0,1,length.out= length(unique(TDat_LMF_plots_Year_EDIT$Year)))) 

Plots_Simple <- TDat_LMF_plots_Year_EDIT %>% dplyr::select(PrimaryKey , Year, EcologicalSiteId) %>%   
...
PlotsPerYear_Total <- ggplot(Plots_Simple , aes(Plots, text = stat(count))) +
 ...
 PlotsPerYear_EcoSite <- ggplot(Plots_Simple_EcoSite , aes(Plots, text = stat(count))) +
                 geom_bar(stat = "count" , position = position_stack(reverse = TRUE) ,
                          aes(fill = Year) , width = .2 , ) +
                 scale_fill_manual(values = Year_Palette) +
                 ggtitle(paste0("Plots Per Year In: " , EcoSiteCaption)) +
                 coord_flip() + theme(axis.text.y = element_blank())
...
SpeciesList <- SpeciesList %>% mutate(link = paste("https://plants.sc.egov.usda.gov/core/profile?symbol=", Species, sep = ""))
...
Merged_Species <- merge(Species_plots_ecosite , SpeciesList , by = c("Species" , "SpeciesState")) %>% 
                  rename(GrowthHabit = GrowthHabit.x , GrowthHabitSub = GrowthHabitSub.x , Duration = Duration.x , 
                                            ...
                                            dplyr::select(Species , ScientificName , PrimaryKey , PlotID ,  
                                                   ...
                                                   mutate(link = paste("https://plants.sc.egov.usda.gov/core/profile?symbol=", Species, sep = ""))
...
Species_cover_summary <- Merged_Species_PercCover %>%  
                         group_by(Species , GrowthHabit , GrowthHabitSub , Duration , Noxious , ScientificName , CommonName , SG.Group, link) %>% 
                         ...
                         dplyr::select(Species , ScientificName , AveragePercentCover , StandardDeviation ,
                                ...
                                Noxious , CommonName , SG.Group, link)
...
Species_Plots_Interactive <-lapply(X = split(Merged_Species_PercCover, list(Merged_Species_PercCover$GrowthHabitSub ,                                                       Merged_Species_PercCover$Duration) , drop = TRUE),
                                    FUN = function(Merged_Species_PercCover){
   current_plot <- ggplot(Merged_Species_PercCover , aes(x = Species , y = AH_SpeciesCover,
                                                         text = paste("PrimaryKey: ", PrimaryKey , 
                                                           "Plot ID: " , PlotID , "Species: " , 
...
PrefForb_Plots_Interactive <-  if (nrow(Merged_Species_SG_PrefForb) < 1) {PrefForb_Plots_Interactive <- NULL} else{
                               ggplot(Merged_Species_SG_PrefForb , aes(x = Species , y = AH_SpeciesCover, 
                                                           ...
                                                           theme_light() +
                                                           labs(y = "Percent Cover") + 
                                                           ggtitle(paste("Percent Cover, Preferred Forbs: " , EcoSiteCaption)) + 
                                                           theme(axis.title.y = element_blank() , 
                                                                 axis.title.x = element_blank(), 
                                                                 axis.text.y = element_blank()) +
                                                                 theme(panel.grid.major.y = element_blank() ,
...
SGGroup_Plots_Interactive <- ggplot(Merged_Species_SG , aes(x = SG.Group , y = AH_SpeciesCover, 
                                                           ...
                                                           scale_y_continuous(limits = c(0 , 100)) +
                                                           theme_light() +
                                                           ggtitle(paste("Percent Cover, Sage-Grouse Group: " , EcoSiteCaption)) + 
...
SageBrush_Plots_Interactive <-  if (nrow(SageBrushCover_Tall) < 1) {SageBrush_Plots_Interactive <- NULL} else{
                               ggplot(SageBrushCover_Tall , aes(x = SageBrushCover , y = Percent , 
                                                           ...
                                                           theme_light() +
                                                           labs(y = "Percent Cover") + 
                                                           ggtitle(paste("Percent Cover, Sagebrush: " , EcoSiteCaption)) + 
                                                           theme(axis.title.y = element_blank() , 
                                                                 axis.text.y = element_blank(), 
                                                                 axis.title.x = element_blank(),
                                                                 axis.ticks.y = element_blank()) +
...
SageBrush_Plots_Static <-  if (nrow(SageBrushCover_Tall) < 1) {SageBrush_Plots_Static <- NULL} else{
                               ggplot(SageBrushCover_Tall , aes(x = SageBrushCover , y = Percent)) +
                                                           ...
                                                           theme(panel.grid.major.y = element_blank(), 
                                                                 axis.text.y = element_blank()) + 
                                                           facet_grid(rows = vars(SageBrushCover), drop = TRUE) + 
                                                           coord_flip()}
...
Nox_Non_Plot_Interactive <- ggplot(data = Nox_Non , (aes(x = Noxious , y = AH_SpeciesCover , 
                                                 ...
                                                 scale_y_continuous(limits = c(0 , 100)) +
                                                 labs(y = "Percent Cover") + 
                                                 ggtitle(paste("Percent Cover, Noxious vs. Non-Noxious Species: " , EcoSiteCaption)) +
...
Tree_SFG_Ly <- ggplotly(SFG_Plots_Interactive[["Tree"]] , tooltip = "text")
Tree_SFG_Ly
```
...
Tree_SFG <- SFG_Plots_Static[["Tree"]]
Tree_SFG
```
...
Succulent_SFG_Ly <- ggplotly(SFG_Plots_Interactive[["Succulent"]] , tooltip = "text")
Succulent_SFG_Ly
```
...
Merged_Species_PercCover_ByPlot$Species <- paste0("<a href='",Merged_Species_PercCover_ByPlot$link,"'>",Merged_Species_PercCover_ByPlot$Species,"</a>")

Merged_Species_PercCover_ByPlot %>% select(-link) %>% DT::datatable(escape = FALSE, extensions = 'Buttons', filter = "top" , options = list(scrollX = TRUE ,
                          dom = 'Bfrtip',
                          buttons =
                          list(list(
                          extend = 'collection',
                          buttons = c('csv', 'excel'),
                          text = 'Download Table'))) , 
                          caption = (paste("Percent Cover by Species by Plot within " , EcoSiteCaption)) , 
                          rownames = FALSE)
```
...
Species_cover_summary %>% select(-link) %>% DT::datatable(escape = FALSE, extensions = 'Buttons', filter = "top" , options = list(scrollX = TRUE ,
                          dom = 'Bfrtip',
                          buttons =
                          list(list(
                          extend = 'collection',
                          buttons = c('csv', 'excel'),
                          text = 'Download Table'))) , 
                          caption = (paste("Average Percent Cover Values Across" , EcoSiteCaption)) , 
                          rownames = FALSE)
```
...
Summary_SFG_State_ByPlot %>% filter(!is.na(GrowthHabitSub)) %>%
                          ...
                          buttons = c('csv', 'excel'),
                          text = 'Download Table'))) ,
                          caption = (paste("Percent Cover by Structure and Functional Group by Plot  within " , EcoSiteCaption)), 
                          rownames = FALSE)
...
Summary_SFG_State %>% filter(!is.na(GrowthHabitSub)) %>%
                          ...
                          buttons = c('csv', 'excel'),
                          text = 'Download Table'))) ,
                         caption = (paste("Percent Cover by Structure and Functional Group in " , EcoSiteCaption)), 
                          rownames = FALSE)
...
NoxNNox_ByPlot %>% filter(!is.na(Noxious)) %>%  DT::datatable(extensions = 'Buttons', filter = "top" , options = list(scrollX = TRUE ,
                          ...
                          buttons = c('csv', 'excel'),
                          text = 'Download Table'))) , 
                          caption = (paste("Percent Cover Noxious Versus Non by Plot within " , EcoSiteCaption)) , 
                          rownames = FALSE)
```
...
NoxNNox %>% filter(!is.na(Noxious)) %>%  DT::datatable(extensions = 'Buttons', filter = "top" , options = list(scrollX = TRUE ,
                          ...
                          buttons = c('csv', 'excel'),
                          text = 'Download Table'))) , 
                          caption = (paste("Percent Cover Noxious Versus Non in " , EcoSiteCaption)) , 
                          rownames = FALSE)
```
...
Summary_Woody_State_ByPlot %>% filter(!is.na(GrowthHabit)) %>% DT::datatable(extensions = 'Buttons', filter = "top" , options = list(scrollX = TRUE ,
                          ...
                          buttons = c('csv', 'excel'),
                          text = 'Download Table'))) , 
                          caption = (paste("Percent Cover Woody vs. Non by Plot within: " , EcoSiteCaption)) , 
                          rownames = FALSE)
```
...
Summary_Woody_State %>% filter(!is.na(GrowthHabit)) %>% DT::datatable(extensions = 'Buttons', filter = "top" , options = list(scrollX = TRUE ,
                          ...
                          buttons = c('csv', 'excel'),
                          text = 'Download Table'))) , 
                          caption = (paste("Percent Cover Woody vs. Non in: " , EcoSiteCaption)) , 
                          rownames = FALSE)
```
...
Summary_SG_Group_ByPlot %>% DT::datatable(extensions = 'Buttons', filter = "top" ,  options = list(scrollX = TRUE ,
                          ...
                          buttons = c('csv', 'excel'),
                          text = 'Download Table'))) , 
                          caption = (paste("Percent Cover by Sage-Grouse Group by Plot within: " , EcoSiteCaption)), 
                          rownames = FALSE)
...
Summary_SG_Group %>% DT::datatable(extensions = 'Buttons', filter = "top" ,  options = list(scrollX = TRUE ,
                          ...
                          buttons = c('csv', 'excel'),
                          text = 'Download Table'))) ,
                          caption = (paste("Percent Cover by Sage-Grouse Group in " , EcoSiteCaption)) , 
                          rownames = FALSE)
...
Summary_PrefForb_ByPlot %>%  DT::datatable(extensions = 'Buttons', filter = "top" ,  options = list(scrollX = TRUE ,
                          ...
                          buttons = c('csv', 'excel'),
                          text = 'Download Table'))) , 
                          caption = (paste("Percent Cover by Preferred Forb By Plot within " , EcoSiteCaption)) , 
                          rownames = FALSE)
...
Summary_PrefForb %>%  DT::datatable(extensions = 'Buttons', filter = "top" ,  options = list(scrollX = TRUE ,
                          ...
                          buttons = c('csv', 'excel'),
                          text = 'Download Table'))) , 
                          caption = (paste("Percent Cover by Preferred Forb in " , EcoSiteCaption)) , 
                          rownames = FALSE)
...
Merged_Species_Trace <- Merged_Species_Trace %>% dplyr::select(Species, ScientificName , GrowthHabit ,
                              GrowthHabitSub , Duration, Noxious , SG.Group, PrimaryKey, PlotID, link)

Merged_Species_Trace$Species <- paste0("<a href='",Merged_Species_Trace$link,"'>", Merged_Species_Trace$Species,"</a>")

Merged_Species_Trace %>% select(-link) %>%  DT::datatable(escape = FALSE, extensions = 'Buttons', filter = "top" , options = list(scrollX = TRUE ,
                          dom = 'Bfrtip',
                          buttons =
                          list(list(
                          extend = 'collection',
                          buttons = c('csv', 'excel'),
                          text = 'Download Table'))) , 
                          caption = (paste("Trace Species by Plot within " , EcoSiteCaption)) , 
                          rownames = FALSE)
...
TraceCover_Table_SpList %>% arrange(Species) %>% select(-link) %>% DT::datatable(escape = FALSE, extensions = 'Buttons', filter = "top" , options = list(scrollX = TRUE ,
                          dom = 'Bfrtip',
                          buttons =
                          list(list(
                          extend = 'collection',
                          buttons = c('csv', 'excel'),
                          text = 'Download Table'))) , 
                          caption = (paste("Trace Species in " , EcoSiteCaption)) , 
                          rownames = FALSE)
```
...
GroundCoverSummary_ByPlot %>%  DT::datatable(extensions = 'Buttons', filter = "top" , options = list(scrollX = TRUE ,
                          ...
                          buttons = c('csv', 'excel'),
                          text = 'Download Table'))) , 
                          caption = (paste("Percent Cover by Plot within " , EcoSiteCaption)) , 
                          rownames = FALSE)
```
...
GroundCoverSummary %>%  DT::datatable(extensions = 'Buttons', filter = "top" , options = list(scrollX = TRUE ,
                          ...
                          buttons = c('csv', 'excel'),
                          text = 'Download Table'))) , 
                          caption = (paste("Average Percent Cover in " , EcoSiteCaption)) , 
                          rownames = FALSE)
```
...
HgtPrep$Species <- as.character(as.factor(HgtPrep$Species))
...
GrassHeightAnnual <- ggplotly(Height_Plot_Species_SFG[["Graminoid.Annual"]] , tooltip = "text", originalData = TRUE) 
GrassHeightAnnual
...
Heights_Summary_SFG_Plot %>%  DT::datatable(extensions = 'Buttons', filter = "top" , options = list(scrollX = TRUE ,
                          ...
                          buttons = c('csv', 'excel'),
                          text = 'Download Table'))) , 
                          caption = paste("Woody and Herbeceous Heights in " , EcoSiteCaption) , 
                          rownames = FALSE)
```
...
Heights_Summary_PerPlot %>%  DT::datatable(extensions = 'Buttons', filter = "top" , options = list(scrollX = TRUE ,
                          ...
                          buttons = c('csv', 'excel'),
                          text = 'Download Table'))) , 
                          caption = (paste("Species Heights by Plot within " , EcoSiteCaption)) , 
                          rownames = FALSE)
```
...
Heights_Summary_Species %>%  DT::datatable(extensions = 'Buttons', filter = "top" , options = list(scrollX = TRUE ,
                          ...
                          buttons = c('csv', 'excel'),
                          text = 'Download Table'))) , 
                          caption = (paste("Heights by Species in " , EcoSiteCaption)) ,
                          rownames = FALSE)
```
...
Heights_Summary_Sagebrush_PerPlot %>%  DT::datatable(extensions = 'Buttons', filter = "top" , options = list(scrollX = TRUE ,
                          ...
                          buttons = c('csv', 'excel'),
                          text = 'Download Table'))) ,
                          caption = (paste("Heights by Sagebrush Species by plots within " , EcoSiteCaption)) ,
                          rownames = FALSE)
```
...
Heights_Summary_Sagebrush %>%  DT::datatable(extensions = 'Buttons', filter = "top" , options = list(scrollX = TRUE ,
                          ...
                          buttons = c('csv', 'excel'),
                          text = 'Download Table'))) , 
                          caption = (paste("Heights by Sagebrush Species in " , EcoSiteCaption)) ,
                          rownames = FALSE)
```
...
EcoSiteGaps_Summary_ByPlot %>%  DT::datatable(extensions = 'Buttons', filter = "top" , options = list(scrollX = TRUE ,
                          ...
                          buttons = c('csv', 'excel'),
                          text = 'Download Table'))) , 
                          caption = (paste("Percent Cover by Canopy Gap Class by Plot within " , EcoSiteCaption)) , 
                          rownames = FALSE)
```
...
EcoSiteGaps_Summary %>%  DT::datatable(extensions = 'Buttons', filter = "top" , options = list(scrollX = TRUE ,
                          ...
                          buttons = c('csv', 'excel'),
                          text = 'Download Table'))) , 
                          caption = (paste("Percent Cover by Canopy Gap Class in " , EcoSiteCaption)) , 
                          rownames = FALSE)
```
